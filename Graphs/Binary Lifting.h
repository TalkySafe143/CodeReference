...
struct BinaryLifting {
    ...
    BinaryLifting(vector<vi>& g, int nn, int root) : n(nn), adj(g) {
    ...
    int walk(int u, int k) { // Walk k edges from u to the root.
        for (int i=0; i < K; i++) if ((1<<i)&k) u = table[i][u];
        return u;
    }

    int lca(int u, int v) {
        if (lvl[u] > lvl[v]) u = walk(u, lvl[u]-lvl[v]);
        if (lvl[v] > lvl[u]) v = walk(v, lvl[v]-lvl[u]);
        if (u == v) return u;
        for (int k = K-1; k>=0; k--) {
    ...
    void dfs(int u, int p, int d) {
        ...
        for (auto v: adj[u]) {
            if (!vis[v] && v != p) {
                dfs(v, u, d+1); parent[v]=u; table[0][v]=u;
            }
        }
    }
};
