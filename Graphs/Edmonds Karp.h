...
struct EdmondsKarp {
  ...
  struct Edge {
    ...
    Edge(int v, ll flow, ll capacity, int residual)
        : v(v), flow(flow), capacity(capacity), residual(residual) {};
  };
  ...
  EdmondsKarp(vector<vector<pair<ll, ll>>> &adj, int n, int s, int t) {
    this->n = n;
    this->s = s;
    this->t = t;
    g.resize(n + 5);
