...
struct AhoCorasick {
  ...
  void buildLinks() {
    ...
    while (len(q)) {
      ...
      int u = trie[wa].link;
      if (trie[u].output) trie[wa].exit = u;
      else trie[wa].exit = trie[u].exit;
...
//      if (ch >= 97) ch = (ch-'a') + 91; // If u use A-Za-z letters
  ...
  void outputChain(int v, function<void(int)> visit) {
    if (trie[v].output) visit(v);
    if (trie[v].exit != -1) outputChain(trie[v].exit, visit);
  }
